...
class BookFolio {
  ...
  _render() {
    this.container.innerHTML = `
      <div class="bpf-wrapper" id="bpf-wrapper">
        ...
        <div class="bpf-loadmore" id="bpf-loadmore" hidden>
          <button class="bpf-loadmore-btn" id="bpf-loadmore-btn" type="button">
            Ver más proyectos <span class="bpf-loadmore-count" id="bpf-loadmore-count"></span>
          </button>
        </div>
  ...
  _renderGrid() {
    ...
    /* Solo se pintan las tarjetas visibles (paginación). */
    const visible = filtered.slice(0, this._visibleCount);

    /* Botón "Ver más": visible solo si quedan proyectos por mostrar. */
    const loadmore    = document.getElementById('bpf-loadmore');
    const loadmoreCnt = document.getElementById('bpf-loadmore-count');
    if (loadmore) {
      const remaining = filtered.length - visible.length;
      loadmore.hidden = remaining <= 0;
      if (loadmoreCnt) loadmoreCnt.textContent = remaining > 0 ? `(+${remaining})` : '';
    }
  ...
  _bindEvents() {
    ...
    document.getElementById('bpf-loadmore-btn')?.addEventListener('click', () => {
      this._visibleCount += this._pageSize;
      this._renderGrid();
    });
...
.bpf-loadmore-btn { display:inline-flex; align-items:center; gap:.5rem; height:44px; padding:0 1.75rem; font-family:var(--font-mono,monospace); font-size:.8rem; letter-spacing:.1em; text-transform:uppercase; color:var(--color-cyan-400,#00e5e5); background:rgba(0,255,255,.04); border:1px solid rgba(0,255,255,.22); border-radius:2px; cursor:pointer; transition:color .18s,border-color .18s,background .18s,box-shadow .18s; }
